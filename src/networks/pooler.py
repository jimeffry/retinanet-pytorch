...
class LevelMapper(object):
    ...
    def __call__(self, boxlists):
        ...
        """
        # Compute level ids
        s = torch.sqrt(torch.cat([(boxes[:,2]-boxes[:,0])*(boxes[:,3]-boxes[:,1]) for boxes in boxlists]))
...
class Pooler(nn.Module):
    ...
    """
    def __init__(self, output_size,num_channels,scales=(1/16.0),sampling_ratio=0,canonical_level=4):
        """
        Arguments:
    ...
    def forward(self, x, boxes):
        ...
        result = torch.zeros(
            (num_rois, self.num_channels,self.output_size[0],self.output_size[1]),
            dtype=dtype,
            device=device,
        )
