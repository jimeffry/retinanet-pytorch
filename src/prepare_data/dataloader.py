...
#from coco_tabel import mscoco2017
sys.path.append(os.path.join(os.path.dirname(__file__),'../configs'))
from config import cfgs

#VOCDataNames = ['person','bicycle','motorbike','car','bus','aeroplane','train','boat']
...
class ReadDataset(Dataset):
    ...
    def __getitem__(self, index):
        img,annot,_,_ = self.pull_item(index)
        sample = {'img': img, 'annot': annot}
        if self.transform:
            sample = self.transform(sample)
        return sample
    ...
    def load_classes(self):
        for idx,tmp in enumerate(cfgs.VOCDataNames):
            self.labels[idx] = tmp
