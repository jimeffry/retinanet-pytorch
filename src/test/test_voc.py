...
def rescale(boxes_f,w,h):
    '''
    bboxes: [[x1,y1,x2,y2]]
    '''
    boxes_f[:,0] = boxes_f[:,0] / float(cfgs.IMGWidth) * w
    boxes_f[:,2] = boxes_f[:,2] / float(cfgs.IMGWidth) * w
    boxes_f[:,1] = boxes_f[:,1] / float(cfgs.IMGHeight) * h
    boxes_f[:,3] = boxes_f[:,3] / float(cfgs.IMGHeight) * h
    return boxes_f

def test_net(net,detector,priors,dataset,use_cuda,train_mode,args):
    '''
    ...
    '''
    labelmap = cfgs.labelmap
    save_folder = cfgs.save_folder
    # top_k = cfgs.top_k
    # thresh = cfgs.score_threshold
    ...
    if os.path.exists(det_file) and train_mode=='test':
        f = open(det_file,'rb')
        ...
        for i in tqdm(range(num_images)):
            ...
            image -= rgb_mean
            image /= rgb_std
            x = Variable(torch.from_numpy(image).permute(2,0,1).unsqueeze(0),requires_grad=False)
            if use_cuda:
                x = x.cuda()
            classification,regression = net(x)
            detections = detector(priors,regression,classification).data
    ...
    #f = open(det_file,'rb')
    #all_boxes = pickle.load(f)
    mmap = evaluate_detections(all_boxes, dataset,labelmap,args)
    return mmap


def evaluate_detections(box_list, dataset,labelmap,args):
    write_voc_results_file(box_list, dataset,cfgs.save_folder,labelmap)
    mmap = do_python_eval(labelmap,args)
    return mmap
...
def do_python_eval(labelmap,args):
    ...
    annopath = cfgs.val_file
    output_dir = cfgs.save_folder
    imgsetpath = None
    cachedir = os.path.join(output_dir, 'gt_cache')
    aps = []
    # The PASCAL VOC metric changed in 2010
    # use_07_metric = args.use_07
    use_07_metric = cfgs.use_07
...
def voc_eval(detfile,annopath,imagesetfile,classname,cachedir,labelmap,args):
    ...
    '''
    ovthresh = cfgs.nms_threshold
    use_07_metric = cfgs.use_07
    score_st = cfgs.conf_threshold
    #get gt
